...
JointBase.prototype = {
     ...
     */
    , before: function(lambdas,advice){
	return this.asPointcut().before(lambdas,advice) ;
    }
    /**
     * after([lambdas,]advice) ;
     */
    , after: function(lambdas,advice){
	return this.asPointcut().after(lambdas,advice) ;
    }
    /**
     * around([lambdas,]advice) ;
     */
    , around: function(lambdas,advice){
	return this.asPointcut().around(lambdas,advice) ;
    }
    /**
     * setter([lambdas,]advice) ;
     */
    , setter: function(lambdas,advice){
	return this.asPointcut().setter(lambdas,advice) ;
    }
    /**
     * getter([lambdas,]advice) ;
     */
    , getter: function(lambdas,advice){
	return this.asPointcut().getter(lambdas,advice) ;
    }
    ...
    , matchPosition: function(astnode,defScope,toplevel,context){
	...
	if(!defScope)
	    ...
	    var scopename = defScope.start.pos + "-" + defScope.end.pos ;

	if(!context.scope[scopename])
	    context.scope[scopename] = {} ;
	if(!context.scope[scopename][this.id])
	    context.scope[scopename][this.id] = {
		matched: 0
	    } ;
	else
	    context.scope[scopename][this.id].matched ++ ;

	return context.scope[scopename][this.id].matched == this.position ;
    }

    , match: function(astnode,toplevel,context) {
	...
	if( !this.matchPosition(astnode,defScope,toplevel,context) )
	    return false ;
...
JointFunctionDefine.prototype.var = function(namePattern,position){
    var joint =  new JointVariable(namePattern,this.aop,this) ;
    joint.pathPatterns = this.pathPatterns ;
    joint.position = position ;
    return joint ;
}
JointFunctionDefine.prototype.const = function(namePattern,position){
    var joint =  new JointConst(namePattern,this.aop,this) ;
    joint.pathPatterns = this.pathPatterns ;
    joint.position = position ;
    return joint ;
}
JointFunctionDefine.prototype.regexp = function(regexp,position){
